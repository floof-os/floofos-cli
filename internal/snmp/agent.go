...
const (
	...
	vppSnmpAgentConfigFile = "/etc/vpp-snmp-agent/config.yaml"
)
...
func EnableSNMP() error {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to enable snmpd: %w\nOutput: %s", err, output)
	}

	cmd = exec.Command("systemctl", "start", snmpdService)
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to start snmpd: %w\nOutput: %s", err, output)
	}
...
func GetSNMPConfig() (string, error) {
	...
	if vppData, err := os.ReadFile(vppSnmpAgentConfigFile); err == nil {
		output.WriteString("\nvpp-snmp-agent config:\n")
		output.WriteString(string(vppData))
	}
...
func SetPollingInterval(interval int) error {
	...
	configDir := "/etc/vpp-snmp-agent"
	if err := os.MkdirAll(configDir, 0755); err != nil {
		return fmt.Errorf("failed to create config directory: %w", err)
	}

	config := fmt.Sprintf(`# VPP SNMP Agent Configuration
# Auto-generated by floofctl

polling_interval: %d
vpp_socket: /run/vpp/cli.sock
vppcfg_path: /etc/vppcfg.yaml
`, interval)

	if err := os.WriteFile(vppSnmpAgentConfigFile, []byte(config), 0644); err != nil {
		return fmt.Errorf("failed to write vpp-snmp-agent config: %w", err)
	}
...
func createSNMPDConfig() error {
	...
	config := `# FloofOS SNMP Configuration
# Auto-generated by floofctl

# Listen on all interfaces
agentaddress 161,udp6:[::1]:161
...
rocommunity public6 default -V systemonly -6
...
# AgentX configuration for VPP SNMP agent
master agentx
...
`
	...
	if err := os.WriteFile(snmpdConfigFile, []byte(config), 0600); err != nil {
		return fmt.Errorf("failed to write snmpd.conf: %w", err)
	}
